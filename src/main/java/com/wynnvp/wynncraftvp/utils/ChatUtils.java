...
public final class ChatUtils {
    ...
    private static String getMessageFromDialogueScreen(String message) {
        ...
        // If the line starts with 3 empty spaces then it's a dialogue choice the player can press on, so we should
        // ignore it
        while (!newLines.isEmpty()
                && (newLines.getFirst().isEmpty() || newLines.getFirst().startsWith("   "))) {
            newLines.removeFirst();
        }
